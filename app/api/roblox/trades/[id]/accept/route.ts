...
// Copy the CSRF token function from the refresh route
...
export async function POST(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Failed to accept trade: ${response.status}`, {
        statusText: response.statusText,
        responseData
      });
      
      return NextResponse.json({ 
        error: `Failed to accept trade: ${response.status} ${response.statusText}`,
        details: responseData
      }, { status: response.status });
    }
    
    console.log(`Successfully accepted trade with ID: ${tradeId}`);
